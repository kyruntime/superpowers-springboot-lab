# Superpowers 学习 README Implementation Plan

> **面向代理式执行者：** 必须使用子技能 `superpowers:subagent-driven-development`（推荐）或 `superpowers:executing-plans`，按任务逐项落实本计划。步骤使用复选框（`- [ ]`）语法以便跟踪进度。

**Goal:** 新增一个中文 `README.md`，面向新手说明本项目和 Superpowers 使用流程。

**Architecture:** 这是文档交付，不修改 Java 代码。README 以“项目简介 + Superpowers 学习笔记 + 本项目案例 + API 和测试命令”的结构组织，使用 Mermaid 展示完整流程。

**Tech Stack:** Markdown、Mermaid、Spring Boot 项目上下文、GitHub README 渲染。
...
- Create: `README.md`  
  GitHub 首页文档，用中文介绍项目、Superpowers、完整流程、常见 skill、项目案例、API 和本地测试。
...
- [ ] **Step 2: 验证 Markdown 关键内容**

Run:

```bash
rg -n "Superpowers|Mermaid|brainstorming|writing-plans|subagent-driven-development|GET    /api/books/\\{id\\}|mvn test" README.md
```

Expected: 命令能找到这些关键内容，说明 README 覆盖了 spec 的核心要求。

- [ ] **Step 3: 提交 README**

```bash
git add README.md
git commit -m "docs: add superpowers learning readme"
```
...
- Spec coverage: 计划覆盖项目简介、Superpowers 解释、完整流程、Mermaid 图、常见 skill、用户常用 skill、内部辅助组件、本项目案例、API 简介和本地测试命令。
- Placeholder scan: 未发现占位式步骤或空泛实现描述。
- Markdown consistency: README 使用 GitHub Markdown 和 Mermaid 代码块，不需要额外渲染工具。

---

## 执行选项

计划已完成并保存至 `docs/superpowers/plans/2026-05-05-superpowers-readme-implementation.md`。有两种执行方式：
